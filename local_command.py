...
#本地接收修改的方法参数，更新字典内的值
def api_dict_update(args1,args2,args3,args4,args5,args6,args7):
    mock_dict['--input_txt']=args1
    mock_dict['--input_wav']=args2
    mock_dict['--output_wav']=args3
    mock_dict['--synthesizer_path']=args4
    rvm_dict['--input-source']=args5
    rvm_dict['--output-composition']=args6
    lip_dict['--outfile']=args7
    api_command_cach()
    pass

#单个任务的信息
class task_info:
    def __init__(self,uuid,cmd,status,mock_dict,rvm_dict,lip_dict,api_dict):
        self.uuid=uuid
        self.cmd=cmd
        self.status=status
        self.mock_dict=mock_dict
        self.rvm_dict=rvm_dict
...
def api_command_cach():
    ...
    for key,value in api_dict.items():
        ...
        sta=None
        task_queue.put(task_info(id,cmd,sta,mock_dict,rvm_dict,lip_dict,api_dict))
        print(task_queue.qsize())
        
    all_task_list_append(task_queue)
    show_task_info()
    #execute_task(0)
    return
...
#执行对应任务，目前在添加到任务队列后自动调用，并根据外部程序执行结果修改执行状态；后续可能修改为被用户自主调用
def execute_task(i):
